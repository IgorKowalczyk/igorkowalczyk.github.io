...
export default function main({ public_repos_data, private_repos_data }) {
 return (
  <Container>
   ...
   <div className="mx-auto -mt-24 flex min-h-screen flex-1 flex-col justify-center motion-reduce:transition-none duration-300 md:w-[90%] xl:w-4/5">
    <div className="md:grid-cols-0 grid lg:grid-cols-5">
      ...
      <div className="mt-4 flex justify-center md:block">
       <NextLink href="/#about"> 
        <a className="arrow animatedLink group relative mt-5 inline-block items-center justify-center p-2 pl-0 pr-0 pb-1 font-semibold" href="#about">
         More about me
         <svg xmlns="http://www.w3.org/2000/svg" className="arrowSymbol inline-block translate-x-[5px] motion-reduce:transition-none duration-200 group-hover:translate-x-[10px]" width="16" height="16" viewBox="0 0 16 16" fill="none">
     ...
     <div className="hidden items-center md:col-span-3 md:mt-8 md:flex lg:col-span-2 smallDesktop:col-span-2 motion-reduce:transition-none">
      <div className="block w-full rounded-md border-[1px] border-[#3391fc]/[40%] p-4 font-poppins text-sm shadow-codeLight transition-colors motion-reduce:transition-none dark:border-white/[15%] dark:bg-[#08152b] dark:shadow-codeDark	">
       <div>
        ...
        <span className="italic">
         <span className="font-semibold text-slate-700 motion-reduce:transition-none duration-200 dark:text-slate-300">$</span>{" "}
         <span>
          list github --user=
       ...
       {config.header.code.lines.map((line, index) => (
        <div key={index}>
         ...
         <span className="italic">
          <span className="font-semibold text-slate-700 motion-reduce:transition-none duration-200 dark:text-slate-300">$</span> <span>{line.command}</span>
         </span>
         <br />
       ...
       <div>
        <span className="font-semibold leading-6 text-[#ea4aaa]">→</span> <span className="font-semibold text-[#66e777]">~/{config.header.code.default.user}</span>{" "}
        <span className="italic">
         <span className="relative font-semibold text-slate-700 motion-reduce:transition-none duration-200 after:absolute after:top-0 after:right-[-1.5em] after:bottom-0 after:my-auto after:animate-cursor after:text-[1em] after:not-italic after:content-['▌'] dark:text-slate-300">$</span>
        </span>
       </div>
      </div>
     </div>
