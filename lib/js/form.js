...
e.preventDefault();
sendfaileddiv.classList.toggle("show-message");
sendsucceededdiv.classList.toggle("show-message");
nameerror.classList.classList.toggle("show-message");
emailerror.classList.classList.toggle("show-message");
messageerror.classList.classList.toggle("show-message");
...
if (name.length < 2) {
error = true;
nameerror.classList.toggle("show-message");
}
if (email.search(/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/) === -1) {
error = true;
emailerror.classList.toggle("show-message");
}
if (message.length < 2) {
error = true;
messageerror.classList.toggle("show-message");
}
if (fatalerror) {
return false;
}
