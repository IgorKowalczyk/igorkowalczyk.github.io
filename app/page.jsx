 ...
 return (
  <>
    ...
    <div className="move-area mx-auto -mt-24 flex min-h-screen flex-1 flex-col justify-center duration-300 motion-reduce:transition-none md:w-[90%] xl:w-4/5">
     <div className="md:grid-cols-0 grid lg:grid-cols-5">
      <div className="md:col-span-3">
       <h1 className="dark:color-black relative m-0 text-center text-[51px] font-black tracking-[-0.03em] text-gray-800 duration-300 dark:text-white md:text-left md:text-[55px] lg:text-[67px] xl:text-[75px]">{header.title}</h1>
      ...
      <div className="hidden items-center motion-reduce:transition-none md:col-span-3 md:-mb-7 md:mt-7 md:flex lg:col-span-2 lg:my-0">
   ...
   <section id="additional-info">
    ...
    <div className="mx-auto mb-8 flex flex-wrap justify-around gap-4 px-8 text-center text-gray-800/70 dark:text-white/70 md:gap-8">
     <Link target="_blank" href={`https://github.com/${meta.accounts.github.username}`} className="flex items-center justify-center gap-2 text-center font-semibold duration-200 hover:text-gray-800 motion-reduce:transition-none dark:hover:text-white">
      <>
       <StarIcon className="mt-[-2px] size-5 stroke-2" aria-hidden="true" role="img" /> <span>{userData && ConvertNumber(userData.userStars)} Stars on repositories</span>
      </>
     </Link>

     <Link target="_blank" href={`https://github.com/${meta.accounts.github.username}`} className="flex items-center justify-center gap-2 text-center font-semibold duration-200 hover:text-gray-800 motion-reduce:transition-none dark:hover:text-white">
      <>
       <svg className="size-5 stroke-2" aria-hidden="true" strokeWidth="1.5" viewBox="0 0 16 16" role="img">
     ...
     <Link target="_blank" href={`https://github.com/${meta.accounts.github.username}`} className="flex items-center justify-center gap-2 text-center font-semibold duration-200 hover:text-gray-800 motion-reduce:transition-none dark:hover:text-white">
      <>
       <svg className="mt-[-2px] size-5 stroke-2" aria-hidden="true" strokeWidth="1.5" viewBox="0 0 32 32" role="img">
     ...
     <Link target="_blank" href={`https://github.com/${meta.accounts.github.username}?tab=followers`} className="flex items-center justify-center gap-2 text-center font-semibold duration-200 hover:text-gray-800 motion-reduce:transition-none dark:hover:text-white">
      <>
       <UsersIcon className="mt-[-2px] size-5 stroke-2" aria-hidden="true" role="img" /> <span>{userData && ConvertNumber(userData.userFollowers)} Github Followers</span>
      </>
     </Link>
    </div>
   </section>
   ...
   <section className="relative my-6 w-full text-center">
    ...
    <Image src={sparkles} alt="Sparkles animation" width="auto" height="auto" className="hide pointer-events-none mx-auto animate-pulse" />
   ...
   <section className="relative mx-auto max-w-5xl">
    ...
    <div className="pointer-events-visible section-fade absolute inset-x-0 bottom-0 z-20 flex pb-8 pt-32 duration-300">
     <div className="flex flex-1 flex-col items-center justify-center duration-200 motion-reduce:transition-none">
      <Link href="/work" className="arrow link group pointer-events-auto relative mt-5 inline-block items-center justify-center p-2 px-0 pb-1 font-semibold duration-200 motion-reduce:transition-none">
       <>
        See more projects
        <svg className="arrowSymbol inline-block translate-x-[5px] duration-200 group-hover:translate-x-[10px] motion-reduce:transition-none" width="16" height="16" viewBox="0 0 16 16" fill="none" alt="Arrow icon">
      ...
      return (
       <div key={`tech-link-${tech.name}`}>
        {tech.link ? (
         ...
         <div key={`tech-nolink-${tech.name}`} className="relative mx-auto flex w-full cursor-pointer items-center justify-between rounded-xl border bg-[#f8f9fa] px-4 py-2 font-semibold text-blue-900 duration-200 hover:bg-[#f2f3f5] motion-reduce:transition-none dark:border-neutral-800 dark:bg-[#161617] dark:text-white dark:hover:border-neutral-700 dark:hover:bg-[#202021]">
          <Image className={cn(tech.class, "h-8 w-8 rounded-md")} loading="lazy" width={32} height={32} src={tech.icon} alt={tech.name} />
          <span className="font-semibold">{tech.name}</span>
         </div>
        )}
   ...
   <section className="mb-12">
    ...
    <div className="relative">
     <div className="relative mx-auto mt-6 grid max-w-5xl grid-cols-1 gap-6 md:grid-cols-2">
      <div className="relative mx-auto mb-4 flex w-full max-w-screen-sm rounded-[10px] border border-black/15 bg-white p-5 shadow-lg duration-200 motion-reduce:transition-none dark:border-neutral-800 dark:bg-[#161617]">
       <Contact />
      </div>
      <div className="space-y-4 p-4">
       <p className="text-xl font-semibold">Or contact me with...</p>
       {contact.links.map((element) => (
        <Link href={element.href} key={`contact-link-${element.href}`} target="_blank" className="group ml-auto mt-2 flex h-12 w-full items-center gap-3 rounded-md border bg-[#f8f9fa] px-4 py-2 text-sm font-semibold text-blue-900 duration-200 hover:bg-[#f2f3f5] motion-reduce:transition-none dark:border-neutral-800 dark:bg-[#161617] dark:text-white dark:hover:border-neutral-700 dark:hover:bg-[#202021]">
         <>
