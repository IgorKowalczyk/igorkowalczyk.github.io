    ...
    {!photos || photos.length === 0 ? (
     ...
     photos.map((result, id) => (
      <Link key={id} href={`/photography/${result.id}`} className="after:content after:shadow-highlight group relative mb-5 block w-full rounded-lg bg-slate-100 bg-cover after:pointer-events-none after:absolute after:inset-0 after:rounded-lg dark:bg-slate-800">
       <Image className="scale-100 transform rounded-lg bg-gray-200 blur-0 duration-200 will-change-auto hover:brightness-90 dark:bg-gray-200/[15%]" src={result.path} alt={result.alt} width={720} height={480} sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, (max-width: 1536px) 33vw, 25vw" />
      </Link>
     ))
    )}
   </div>
