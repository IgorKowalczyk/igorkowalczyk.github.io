 ...
 return (
  <div id={repo.name} className="rounded-[10px] border-[1px] border-black/[15%] bg-white p-5 duration-200 hover:scale-105 hover:shadow-xl motion-reduce:transition-none motion-reduce:hover:scale-100 dark:border-white/[15%] dark:bg-[#08152b]">
   <div className="text-left">
    <NextLink href={repo.url} key={repo.id} className="break-all text-left font-poppins font-semibold" target="_blank">
   ...
   {repo.repositoryTopics ? (
    <div className="text-left">
     {repo.repositoryTopics.edges.map((topic) => (
      <NextLink key={topic.node.topic.name} href={`https://github.com/topics/${topic.node.topic.name}`} target="_blank">
   ...
   <div className="hide-scrollbar mt-2 flex gap-1 overflow-hidden overflow-x-auto">
    <NextLink key="repo_lang" href={`${repo.url}/search?l=${repo.primaryLanguage ? repo.primaryLanguage.name : "Markdown"}`} target="_blank" aria-label={`${repo.primaryLanguage ? repo.primaryLanguage.name : "Markdown"} search`}>
    ...
    <NextLink key="repo_stars" href={`${repo.url}/stargazers`} target="_blank">
    ...
    <NextLink key="repo_forks" href={`${repo.url}/network/members`} target="_blank">
     <span className="flex w-max snap-center snap-always content-center items-center rounded-lg border-2 border-transparent bg-black/[5%] py-[0.12em] px-[0.5em] align-middle text-[88%] text-black/[60%] duration-200 motion-reduce:transition-none dark:bg-white/10 dark:text-white/[70%]">
