...
export function RepoCard(repo) {
 return (
  <div id={repo.name} className="card z-10 rounded-[10px] border-[1px] border-black/[15%] bg-white p-5 duration-200 ease-in-out hover:shadow-xl motion-reduce:transition-none dark:border-white/[15%] dark:bg-[#08152b]">
   <div>
    <div className="text-left">
     ...
     {"  "}
     <span className="ml-1 inline-flex content-center items-center rounded-lg border-[1px] border-black/[15%] px-[0.5em] py-[0.12em] align-middle text-[88%] text-black/[55%] duration-200 hover:bg-black/[5%] motion-reduce:transition-none dark:border-white/[15%] dark:text-white/[50%] dark:hover:bg-white/[5%]">
      {repo.isArchived ? (
       <>
     ...
     <div className="text-left">
      {repo.repositoryTopics.edges.map((topic) => (
       <Link key={topic.node.topic.name} href={`https://github.com/topics/${topic.node.topic.name}`} target="_blank">
        <span className="mr-[10px] mt-1 inline-flex content-center items-center rounded-[2em] border-[1px] border-black/[15%] px-[0.5em] py-[0.12em] align-middle text-[88%] text-black/[60%] duration-200 hover:bg-black/[5%] motion-reduce:transition-none dark:border-white/[15%] dark:text-white/[50%] dark:hover:bg-white/[5%]">#{topic.node.topic.name}</span>
       </Link>
      ))}
     </div>
    ) : (
    ...
    <div className="hide-scrollbar flex gap-1 overflow-hidden overflow-x-auto">
     <Link key="repo_lang" href={`${repo.url}/search?l=${repo.primaryLanguage ? repo.primaryLanguage.name : "Markdown"}`} target="_blank" aria-label={`${repo.primaryLanguage ? repo.primaryLanguage.name : "Markdown"} search`}>
      <span className="my-1 flex w-max content-center items-center rounded-lg border-2 border-transparent bg-black/[5%] px-[0.5em] py-[0.12em] text-[88%] text-black/[60%] duration-200 hover:bg-black/10 motion-reduce:transition-none dark:bg-white/10 dark:text-white/[70%] dark:hover:bg-white/20">
       <span
     ...
     <Link key="repo_stars" href={`${repo.url}/stargazers`} target="_blank">
      <span className="my-1 flex w-max snap-center snap-always content-center items-center rounded-lg border-2 border-transparent bg-black/[5%] px-[0.5em] py-[0.12em] align-middle text-[88%] text-black/[60%] duration-200 hover:bg-black/10 motion-reduce:transition-none dark:bg-white/10 dark:text-white/[70%] dark:hover:bg-white/20">
