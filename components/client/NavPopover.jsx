...
export default function NavPopover(props) {
 return (
  <Popover {...props}>
   <>
    <Popover.Button className="group relative hidden rounded-lg p-1 text-gray-700 transition-all duration-200 hover:bg-black/10 hover:text-gray-800 ui-open:bg-black/10 ui-open:text-gray-800 motion-reduce:transition-none sm:px-3 sm:py-2 md:flex md:items-center dark:text-neutral-400 dark:hover:bg-white/10 dark:hover:text-neutral-200 dark:ui-open:bg-white/10 dark:ui-open:text-neutral-200">
     <span>More</span>
    ...
    <Transition enter="transition ease-out duration-100 motion-reduce:transition-none" enterFrom="transform opacity-0 duration-100 scale-95 motion-reduce:transition-none" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-100 motion-reduce:transition-none" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
     <Popover.Panel className="absolute left-1/2 z-10 mt-3 w-screen max-w-sm -translate-x-1/2 transform rounded-lg border-[1px] border-black/10 bg-white px-4 opacity-100 shadow-lg backdrop-blur-xl duration-100 sm:px-0 md:!bg-opacity-70 dark:border-neutral-800 dark:bg-[#161617] ">
      <div className="relative p-3">
       <Link target="_blank" href={`https://github.com/${meta.accounts.github.username}`} className="flex items-center rounded-lg p-3 transition hover:bg-blue-400/20 dark:hover:bg-white/5">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-400/20 text-white sm:h-12 sm:w-12 dark:bg-white/10 dark:text-neutral-800">
