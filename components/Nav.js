  ...
  <div className="fixed top-0 z-[100] mx-0 mt-0 w-full font-poppins shadow backdrop-blur-[8px] dark:shadow-2xl">
   <nav className="dark:bg-opacity-70% relative mx-auto flex w-full items-center justify-between border-b-[1px] border-white/[15%] bg-white bg-opacity-70 pt-4 pb-4 dark:bg-[#08152b] dark:bg-opacity-70">
    ...
    <Tippy content={resolvedTheme === "dark" ? "Enable light theme" : "Enable dark theme"} animation={"shift-away"} theme={"translucent"}>
     <button aria-label="Toggle Dark Mode" type="button" className="mx-8 flex h-9 w-9 items-center justify-center rounded-lg border-2 border-transparent bg-transparent transition-all duration-300 hover:border-[#5686f5] hover:shadow-hover dark:bg-white/10" onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}>
      {mounted && (
