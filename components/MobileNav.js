...
export default function MobileMenu() {
 ...
 return (
  <>
   ...
   {isMenuMounted && (
    <div className={cn(isMenuRendered && "menuRendered", "absolute left-0 top-0 z-[1001] mt-[73px] flex h-screen w-3/4 flex-col opacity-0 backdrop-blur-[9px] duration-200 motion-reduce:transition-none md:hidden ")}>
     <div className="h-full border-r-[1px] border-black/[10%] bg-white bg-opacity-70 px-3.5 shadow duration-200 firefox:bg-opacity-100 motion-reduce:transition-none dark:border-white/[15%] dark:bg-[#08152b] dark:bg-opacity-70 dark:shadow-2xl dark:firefox:bg-opacity-100">
      <div>
       {menuItems.array.map((item, index) => {
        return (
         <p key={index} className="mt-6 w-0 translate-x-[-16px] whitespace-nowrap border-b border-gray-300 text-sm font-semibold text-gray-900 opacity-0 duration-300 motion-reduce:transition-none dark:border-gray-700/75 dark:text-gray-100" style={{ transitionDelay: `${150 * index - 50}ms` }}>
          <Link href={item.href} key={index}>
           <a className="flex w-auto pb-4" onClick={toggleMenu} target={item.target}>
            {item.title}
           </a>
          </Link>
         </p>
